export function calculateSprint30mScore(time: number): number {
  ...
  // (3.70, 4.60]: each 0.02s bucket = -1 point, from 79 down to 35
  if (time <= 4.60 + EPS) {
    const steps = Math.ceil((time - 3.70) / 0.02 - EPS);
    return Math.max(80 - steps, 35);
  }

  // (4.60, 5.20]: each 0.04s bucket = -1 point, from 34 down to 20
  if (time <= 5.20 + EPS) {
    const steps = Math.ceil((time - 4.60) / 0.04 - EPS);
    return Math.max(35 - steps, 20);
  }

  // (5.20, 5.50]: each 0.05s bucket = -1 point, from 19 down to 14
  if (time <= 5.50 + EPS) {
    const steps = Math.ceil((time - 5.20) / 0.05 - EPS);
    return Math.max(20 - steps, 14);
  }

  // (5.50, 5.90]: each 0.10s bucket = -1 point, from 13 down to 10
  if (time <= 5.90 + EPS) {
